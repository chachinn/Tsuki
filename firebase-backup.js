...
async function readLocalAsset(key) {
  if (typeof window.appearanceAssetGet !== "function") return null;
  try {
    const value = await window.appearanceAssetGet(key);
...
async function ensureConsent(settings) {
  ...
  const accepted = window.confirm(
    "Cloud Backup contains private cycle, pregnancy, wellness, journal and locally saved media data. It is protected by your Firebase account and private Security Rules, but it is not end-to-end encrypted. Enable cloud backup?"
  );
...
async function restoreAsset(uid, assetId) {
  ...
  if (typeof window.appearanceAssetPut !== "function") return false;
  const blob = new Blob(parts, { type: meta.type || "application/octet-stream" });
  await window.appearanceAssetPut(meta.key, blob);
  return true;
}
...
async function restoreSelectedBackup() {
  ...
  try {
    ...
    if (currentRaw && typeof window.appearanceAssetPut === "function") {
      await window.appearanceAssetPut(TSUKI_RECOVERY_KEY, currentRaw).catch(() => {});
    }
